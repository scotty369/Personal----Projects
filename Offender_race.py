...
location_data = pd.read_csv('Offender_race.csv')

fig = px.bar(location_data, x='key', y='value', 
             title="Offender race Distribution",
             labels={'key': 'Offender race', 'value': 'Count'},
...
plt.title('Correlation Heatmap for Offender race')
plt.show()

fig = px.scatter(location_data, x='key', y='value', 
                 title="Scatter Plot: Offender race vs. Value",
                 labels={'key': 'Offender race', 'value': 'Value'},
...
fig = px.box(location_data, x='key', y='value', points="all",
             title="Value Distribution Across Offender race",
             labels={'key': 'Offender race', 'value': 'Value'})
fig.show()
