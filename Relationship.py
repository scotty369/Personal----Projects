...
location_data = pd.read_csv('Relationship.csv')

fig = px.bar(location_data, x='key', y='value', 
             title="Relationship Distribution",
             labels={'key': 'Relationship Type', 'value': 'Count'},
...
plt.title('Correlation Heatmap for Relationships')
plt.show()

fig = px.scatter(location_data, x='key', y='value', 
                 title="Scatter Plot: Relationship vs. Value",
                 labels={'key': 'Relationship Type', 'value': 'Value'},
...
fig = px.box(location_data, x='key', y='value', points="all",
             title="Value Distribution Across Relationship Types",
             labels={'key': 'Relationship Type', 'value': 'Value'})
fig.show()
